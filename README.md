...
### 오차는 어디에 몰려 있나 — `scripts/error_analysis.py`

검증 160건의 오차를 조건별로 나눠 보면 한쪽에 몰려 있다.

| 조건 | 건수 | 평균 오차 | 최대 오차 |
|---|---:|---:|---:|
| 보이드 없음 | 32 | **0.15℃** | 0.58℃ |
| 보이드가 핫스팟을 덮음 | 54 | **1.74℃** | **11.21℃** |
| 가까움 (간격 0~1mm) | 27 | 1.07℃ | 4.90℃ |
| 멀리 (간격 1mm 초과) | 47 | 0.64℃ | 2.53℃ |

보이드가 없으면 거의 정확하고, 보이드가 핫스팟을 덮어 열 경로가 막히는 조건에서 오차가 커진다.
실제 최고 온도 80~120℃ 구간의 평균 오차가 2.92℃ 로 가장 크다. 상대 오차로는 평균 3.9%.

![오차 분석](figures/error_analysis.png)
